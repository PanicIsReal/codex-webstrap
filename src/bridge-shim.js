(() => {
  ...
  // Polyfill crypto.randomUUID for non-secure contexts (e.g. VPN IPs over plain HTTP).
  // crypto.randomUUID is only available in secure contexts (HTTPS or localhost), but
  // crypto.getRandomValues is available everywhere, so we use it as a fallback.
